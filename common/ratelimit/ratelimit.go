...
type Bucket struct {
	...
	sessionToken map[uint32]int64

	Direct string

	maxWaitQueue time.Duration
}
...
func NewBucketWithQuantumAndClock(fillInterval time.Duration, capacity, quantum int64, clock Clock) *Bucket {
	...
	return &Bucket{
		...
		sessionToken:    make(map[uint32]int64),
		maxWaitQueue:    5e9,
	}
}

func (tb *Bucket) SetMaxWaitQueue(maxWaitQueue time.Duration) {
	tb.maxWaitQueue = maxWaitQueue
}

func (tb *Bucket) GetMaxWaitQueue() time.Duration {
	return tb.maxWaitQueue
}

func (tb *Bucket) resetToken(sid uint32) {
	tb.mu.Lock()
	defer tb.mu.Unlock()

	v, ok := tb.sessionToken[sid]

	if ok {
		fmt.Printf("Current tokes %d  cache tokens %d.\n", tb.availableTokens, v)
		tb.availableTokens -= v
		fmt.Printf("Relese after tokes %d.\n", tb.availableTokens)
		delete(tb.sessionToken, sid)
		//return
	}
...
func (tb *Bucket) Signal(sid uint32) {
	tb.resetToken(sid)
	tb.notifier.Signal()
}
...
// available.
func (tb *Bucket) Wait(count int64, sid uint32, direct string) bool {
	d := tb.TakeV2(count, direct, sid)

	//if d > 0 {
	//	fmt.Printf("Session id %d. Wait %d Sec. Direct %s\n", sid, (d / 1e9), direct)
	//	select {
	//	case <-tb.notifier.Wait():
	//		//fmt.Printf("%s, Intrruput ......\n", time.Now().Format("2006-01-02 15:04:05.000"),)
	//
	//		//tb.startTime = tb.clock.Now()
	//		//tb.startTime.Add(time.Duration(endTick) * tb.fillInterval)
	//	case <-time.After(d):
	//		//fmt.Printf("%s, Time out ......\n",time.Now().Format("2006-01-02 15:04:05.000"),)
	//	}
	//}

	if d > tb.maxWaitQueue {
		return true
	}
	tb.clock.Sleep(d)
	return false

	//if d := tb.TakeV2(count, direct); d > 0 {
	//	fmt.Printf("Session id %d. Wait %d Sec. Direct %s\n", sid, (d / 1e9), direct)
	//	tb.clock.Sleep(d)
	//}
}

func (tb *Bucket) Wait2(count int64) bool {
...
func (tb *Bucket) TakeV2(count int64, direct string, sid uint32) time.Duration {
	tb.mu.Lock()
	defer tb.mu.Unlock()
	d, _ := tb.takeV2(tb.clock.Now(), count, infinityDuration, direct)

	//fmt.Printf("Get token %d session id %d.\n", count, direct)
	//if count > 0  {
	//	tb.sessionToken[sid] = count
	//}
...
func (tb *Bucket) take(now time.Time, count int64, maxWait time.Duration) (time.Duration, bool) {
	...
	//fmt.Printf("current start time %s, %d\n", tb.startTime.Format("2006-01-02 15:04:05.000"), (time.Duration(endTick) * tb.fillInterval))

	endTime := tb.startTime.Add(time.Duration(endTick) * tb.fillInterval)

	//fmt.Printf("current start time %s\n", tb.startTime.Format("2006-01-02 15:04:05.000"))
	waitTime := endTime.Sub(now)

	//fmt.Printf("----------->>>>>>>>>>  Wait time %d Sec\n", waitTime / 1e9)

	if waitTime > maxWait {
		//fmt.Printf("----------->>>>>>>>>>  Ignore Wait time %d Sec\n", waitTime / 1e9)
		return 0, false
	}
	tb.availableTokens = avail
...
func (tb *Bucket) takeV2(now time.Time, count int64, maxWait time.Duration, direct string) (time.Duration, bool) {
	if count <= 0 {
		return 0, true
	}
	...
	avail := tb.availableTokens - count
	fmt.Printf("Must available %d. Direct %s\n", avail, direct)
	if avail >= 0 {
		tb.availableTokens = avail
		return 0, true
	}
